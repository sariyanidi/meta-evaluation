...
odir = 'figures/TMP2%s-%s/%.4f/' % (dbtype, mean_or_median, imean)
...
ecs += ['NICPR2auto']
#ecs += [ErrorComputerChamferFLR_NIC()]
ecs += [ErrorComputerChamfer(correction='pair-mixed-r2-sqrt')]
...
for m in range(len(methods)):
    ...
    figpath_png = '%s/subjectwise_err_%d.png' % (odir, m+1)
    ...
    for method in methods:
        ...
        for dbname in dbnames:
            ...
            if type(err_computer) is ErrorComputerLandmarks:
                tmp = pr0.compute_avg_err_on_dataset(err_computer, method, Nsubj, median=median, mm=False)
            else:
                tmp = pr.compute_avg_err_on_dataset(err_computer, method, Nsubj, median=median, mm=False)
            errs_per_ec[ei] += tmp
            ecm += tmp
...
for ei, ej in pairs:
    ...
    plt.savefig(figpath)
    figpath_png = '%s/subjectwise_err_%d.png' % (odir, fix)
    plt.savefig(figpath_png)
# plt.show()
#    break
